...
#if defined(NDEBUG)
# error "Shijiancoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Shijiancoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Shijiancoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Shijiancoin: find block wanted by given orphan block
...
{
    int64_t nSubsidy = 0.000001 * COIN;
    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 16 * 60;  // 16 mins

// Shijiancoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Shijiancoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Shijiancoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Shijiancoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Shijiancoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Shijiancoin: track money supply and mint amount info
...
// Shijiancoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Shijiancoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Shijiancoin: compute stake entropy bit for stake modifier
    ...
    // Shijiancoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Shijiancoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Shijiancoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Shijiancoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Shijiancoin-loadblk");
